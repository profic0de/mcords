...
class Logger:
    ...
    def _format_message(self, level, *args):
        ...
        color = {"DEBUG":"gray","INFO":"white","WARN":"yellow","PRINT":"white","ERROR":"red"}[level.upper()]

        return f"{colored(f"[{timestamp}]","blue")} {colored(f'[{self.thread_name}/{level.upper()}]: {' '.join(str(arg) for arg in args)}',color=color)} "
